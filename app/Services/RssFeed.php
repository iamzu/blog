...
class RssFeed
{
    ...
    protected function buildRssData()
    {
        ...
        $posts = Post::query()->where('published_at', '<=', $now)
            ->where('is_draft', 0)
        ...
        $feed = str_replace(array('<rss version="2.0">', '<channel>'), array('<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">', '<channel>' . "\n" . '    <atom:link href="' . url('/rss') .
